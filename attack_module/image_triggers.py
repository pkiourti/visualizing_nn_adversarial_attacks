...
class RandomRectangularPattern(ImageEntity):
    ...
    def create(self) -> None:
        ...
        if self.color_algorithm == 'channel_assign':
            ...
            self.pattern = (self.random_state_obj.random_sample(num_elem_to_generate).\
                reshape((self.num_rows, self.num_cols, self.num_chan)) * dtype_max_val).astype(self.dtype)
        else:
